...
class NeuralNetwork:
    def __init__(self, layer_dims, activation_functions):
        self.layers = [Layer(layer_dims[i - 1], layer_dims[i], activation_functions[i - 1]) for i in range(1, len(layer_dims))]
    ...
    def compute_cost(self, AL, Y, lambda_reg=0.1):
        ...
        L2_cost = sum(np.sum(np.square(layer.W)) for layer in self.layers)  # Removed unnecessary square of bias
...
y_train = y_train.reshape(-1, 1)  # Ensuring Y is a 2D array
y_test = y_test.reshape(-1, 1)

# Define network architecture and initialize the neural network
nn = NeuralNetwork(layer_dims=[784, 10, 8, 8, 4, 1], activation_functions=['relu', 'relu', 'relu', 'relu', 'sigmoid'])

# Train the neural network
nn.train(x_train, y_train, learning_rate=0.001, num_iterations=2500, print_cost=True)

# Perform a forward pass on the test set and calculate accuracy
